...
describe("generateClearDefaultBindings", () => {
  ...
  it("should handle multiple custom commands", () => {
    ...
    expect(result[3]!.when).toBeUndefined();
  });

  it("should handle empty default commands", () => {
    const result = generateClearDefaultBindings("ctrl+t", [], [{ name: "myCommand" }]);
  ...
  it("should handle empty custom commands", () => {
    const result = generateClearDefaultBindings("ctrl+u", ["defaultCommand"], []);
